...
def main():
  ...
  @jit
  def forward_loss_batch(variables, s, sr):
    '''shape of input: batch is in the first (0) dimension'''
    s_hat = model.apply(variables, s, sr)
    return jnp.mean(batch_audspec_loss(s, s_hat))
  
  def update_batch_sr(variables, sr, ov, osr, opt_state_v, opt_state_sr, x): 
    ...
    loss, (g_v, g_sr) = value_and_grad(forward_loss_batch, (0, 2))(variables, s, sr)
  ...
  train_set = AudioDataset(home_dir=config.home_dir, partition=config.partition, sampling=1.)
  ...
  for x in train_data_loader:
    ...
    variables, sr, opt_state_v, opt_state_sr, l = update_batch_sr(
      variables, sr, op_v, op_sr, opt_state_v, opt_state_sr, x)
    total_loss += l
